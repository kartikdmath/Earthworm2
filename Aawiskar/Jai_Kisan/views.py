...
def booked_change_status(request):
    totalitem = 0
    if request.method == 'GET':
        # prod_id = request.GET['prod_id']
        if request.user.is_authenticated:
            totalitem = len(VendorProduct.objects.filter(user=request.user))
            Vender_product_list = VendorProduct.objects.filter(user=request.user)
            amount = 0.0
            technician = 50.0
            fuel_charges = 100.0
            totalamount = 0.0
            ...
            if cart_product:
                for p in cart_product:
                    ...
                    amount += tempamount
                    totalamount = amount + technician + fuel_charges
            print(Vender_product_list,"VMURL")
            pr=0
            pr_1d=0
            Product_list=''
            for vpl in Vender_product_list:
                pr=vpl.product
                pr_id =vpl.id
                print(pr_id)
                print(pr)
                Product_list = Booked.objects.filter(Add_product_id=pr_id,product=pr)
            ...
            print(cart_product)
            if Product_list:
                return render(request, 'Jai_Kisan/booked_change_status.html',
                              {'Product_list': Product_list, 'cart_product': cart_product, 'totalitem': totalitem, 'totalamount':totalamount})
            else:
                return render(request, 'Jai_Kisan/booked_change_status.html', {'totalitem': totalitem})
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            fuel_charges = 100.0
            totalamount = 0.0
            cart_product1 = [p for p in Item.objects.all() if p.user == request.user]

            print("Hey")
            if cart_product1:
                for p in cart_product1:
                    tempamount = (p.duration * p.product.discounted_price)
                    amount += tempamount
                    totalamount = amount + technician + fuel_charges
            print(Vender_product_list, "VMURL")
            ...
            for vpl in Vender_product_list:
                ...
                print(pr_id)
                print(pr)
                Product_list = Booked.objects.filter(product=pr).update(status=status)

            messages.success(request, 'Congratulations!! Order update sucessfully')
            Product_list = Booked.objects.filter(product=pr)
            cart_product = [p for p in VendorProduct.objects.all() if p.user == request.user]
            # print(cart_product)
            if Product_list:
                return render(request, 'Jai_Kisan/booked_change_status.html',
...
class ProductDetailView(View):
    def get(self, request, pk):
        ...
        user=request.user
        # vender_add = vendor.objects.filter(pk='Murlidea')
        # print(vender_add)
        vender_product_details=VendorProduct.objects.filter(product=pk)
        print(vender_product_details)
        for vid in vender_product_details:
            vidd=vid.id
            print(vidd)
        print(product.id)
        ...
        return render(request, 'Jai_Kisan/productdetail.html',
                      {'product': product, 'item_already_in_cart': item_already_in_cart,
                       'vender_product_details':vender_product_details, 'totalitem': totalitem})
...
def add_to_cart(request):
    ...
    print(product,"murlideb")
    item_already_in_cart1 = Item.objects.filter(Q(Add_product_id=product) & Q(user=request.user)).exists()
    vender_product_details = VendorProduct.objects.filter(id=product, user=user)
    print(vender_product_details)
    vidd=0
    for vid in vender_product_details:
        vidd = vid.product
    ...
    if item_already_in_cart1 == False:
        ...
        print("debug",user)
        Item(user=user,Add_product_id_id=product, product=vidd).save()
...
def payment_done(request):
    ...
    for cid in cartid:
        ...
        print(cid.Add_product_id)
        Booked(user=user, Add_product_id=cid.Add_product_id,product=cid.product).save()
        print("Order Saved")
        cid.delete()
...
@login_required
def vendorportal(request):
    return render(request, 'Jai_Kisan/vendorportal.html')
